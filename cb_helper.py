...
def get_json_file(filename='cb_boxes.json'):
    """
    Build the JSON file path based on the installation directory.

    Args:
        filename (str): the name of the JSON file to save.
            cb_boxes (default) for the nodes boxes,
...
def load_boxes(filename='cb_boxes'):
    """
...
def save_boxes(boxes, filename='cb_boxes'):
    """
    Save the updated dict to the JSON file. Create a new one if it doesn't exist.

    Args:
        boxes (dict): a dict separated by Groups, where each group have a list of nodes and settings.
        filename (str): the name of the JSON file to save, default to 'cb_boxes.
            cb_boxes for the nodes boxes,
            cb_settings for the widget settings.
    """
    json_file = get_json_file(filename)

    with open(json_file, 'w') as wf:
        json.dump(boxes, wf, indent=4)
